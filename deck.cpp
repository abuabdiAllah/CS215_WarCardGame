...
{
    deck.clear();
    for (int val = CARD_START; val < CARD_START + POINTS; val++) //val goes from 2 to 14
    {
...
Card Deck::deal_a_card()
{
    ...
    {
        Card result = deck.back(); //this takes a card from the tail of the deck and returns it and removes it from the deck vector so it cannot be dealt again
        deck.pop_back();
        return result;
    }
}
